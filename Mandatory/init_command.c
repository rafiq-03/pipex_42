...
/*   Updated: 2024/05/13 19:25:46 by rmarzouk         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	first_command(t_pipex *pipex, char *av_2)
{
	...
	if (ft_strlen(av_2) == 0)
		return ;
	first->command = ft_split(av_2, ' ');
	// I want to make a split function for this shit
	if (!first->command)
	{
		//check it later
	}
...
void	other_commands(t_pipex *pipex, char **av, int ac)
{
	...
	while (++pipex->j < ac - 1)
	{
		...
		node = ft_lstnew();
		node->command = ft_split(av[pipex->j], ' ');
		// I want to make a split function for this shit
		...
		if (!ft_strchr(node->c_name, '/'))
		{
			...
			{
				node->command[0] = ft_strjoin(pipex->path[pipex->i],
												pipex->tmp1);
